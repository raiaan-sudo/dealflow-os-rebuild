...
create index if not exists system_jobs_unreviewed_issue_idx
  on public.system_jobs (status, dead_lettered_at, created_at desc)
  where reviewed_at is null;

create index if not exists stripe_webhook_events_unreviewed_failed_idx
  on public.stripe_webhook_events (status, created_at desc)
  where reviewed_at is null;

insert into public.app_schema_metadata (key, value)
values ('schema_version', '20260429100000')
