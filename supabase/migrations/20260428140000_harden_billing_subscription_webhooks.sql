alter table public.billing_subscriptions
  ...
  add column if not exists stripe_latest_event_created bigint not null default 0;

create index if not exists billing_subscriptions_latest_event_idx
  on public.billing_subscriptions (stripe_latest_event_created desc);

comment on column public.billing_subscriptions.stripe_latest_event_id is
  'Most recent Stripe event ID applied to this billing subscription row.';

comment on column public.billing_subscriptions.stripe_latest_event_created is
  'Most recent Stripe event created timestamp applied to this billing subscription row; older events are ignored.';

drop function if exists public.apply_billing_subscription_webhook(
  uuid,
  uuid,
  text,
  text,
...
begin
  ...
  where coalesce(public.billing_subscriptions.stripe_latest_event_created, 0) <=
...
values ('schema_version', '20260428140000')
