...
alter table public.user_credit_ledger
  drop constraint if exists user_credit_ledger_balance_after_nonnegative;

comment on table public.user_credits is
  'Per-user paid generation credit balance. Balance is stored in cents-equivalent integer units and may go negative when DealFlow fronts paid generation.';

comment on table public.user_credit_ledger is
  'Append-only credit movements for top-ups, paid generation consumption, overdrafts, and refunds.';
...
values ('schema_version', '20260510014500')
