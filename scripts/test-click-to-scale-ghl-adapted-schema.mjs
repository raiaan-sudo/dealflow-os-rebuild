...
assert.match(ghlClient, /addContactToWorkflow/);
assert.match(ghlClient, /\/contacts\/\$\{encodeURIComponent\(payload\.contactId\)\}\/workflow\/\$\{encodeURIComponent\(payload\.workflowId\)\}/);
...
assert.match(crmSyncService, /export async function readPartnerGhlWorkflowConfig/);
...
assert.match(crmSyncService, /maybeEnrollContactInWorkflow/);
assert.match(crmSyncService, /isGhlWorkflowEnrollmentEnabled/);
assert.match(crmSyncService, /\.from\("partner_ghl_workflow_config"\)/);
assert.match(crmSyncService, /\.addContactToWorkflow\(\{/);
assert.match(crmSyncService, /workflow_enrollment_disabled/);
assert.match(crmSyncService, /workflow_config_missing/);
assert.match(crmSyncService, /workflow_waiting_for_opportunity_sync/);
assert.match(crmSyncService, /workflow_enrolled/);
assert.match(crmSyncService, /workflow_enrollment_failed/);
...
assert.match(provisioningService, /\.from\("partner_ghl_workflow_config"\)/);
...
assert.match(provisioningService, /\.getWorkflows\(/);
...
assert.match(partnerCrmSyncLiveContactProofRoute, /WORKFLOW_V1_PROOF_RUN_ID/);
assert.match(partnerCrmSyncLiveContactProofRoute, /qa\+ghl-workflow-v1-20260618@example\.com/);
assert.match(partnerCrmSyncLiveContactProofRoute, /mode: z\.enum\(\["fullSync", "workflowRetry"\]\)/);
assert.match(partnerCrmSyncLiveContactProofRoute, /Workflow retry mode is only allowed for the approved workflow proof/);
assert.match(partnerCrmSyncLiveContactProofRoute, /GHL contact writes must remain disabled for workflow retry proof/);
assert.match(partnerCrmSyncLiveContactProofRoute, /GHL opportunity writes must remain disabled for workflow retry proof/);
assert.match(partnerCrmSyncLiveContactProofRoute, /runWorkflowRetryProof/);
assert.match(partnerCrmSyncLiveContactProofRoute, /\.addContactToWorkflow\(\{/);
assert.match(partnerCrmSyncLiveContactProofRoute, /contactCreatedOrUpdated: false/);
assert.match(partnerCrmSyncLiveContactProofRoute, /opportunityCreatedOrUpdated: false/);
assert.match(partnerCrmSyncLiveContactProofRoute, /GHL workflow enrollment is not enabled for this proof/);
...
assert.match(partnerCrmSyncLiveContactProofRoute, /workflowEnrollment: Boolean/);
