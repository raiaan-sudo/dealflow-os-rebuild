...
const mappingRepairRoute = readFileSync("src/app/api/admin/click-to-scale/ghl-mapping-repair/route.ts", "utf8");
const readinessScript = readFileSync("scripts/check-click-to-scale-ghl-production-readiness.mjs", "utf8");
const reconcileScript = readFileSync("scripts/reconcile-click-to-scale-ghl-workspaces.mjs", "utf8");
const reviewFailuresScript = readFileSync("scripts/review-click-to-scale-ghl-provisioning-failures.mjs", "utf8");
...
assert.match(provisioningService, /readLatestSuccessfulLocationEvent/);
assert.match(provisioningService, /location_reuse/);
assert.match(provisioningService, /previous_location_create_succeeded/);
assert.match(provisioningService, /ghl_user_invite_non_blocking_failed/);
assert.match(provisioningService, /non_blocking: true/);
assert.match(provisioningService, /ghl_user_invite_status/);
...
assert.match(packageJson, /ghl:proof-lead-sync/);
assert.match(packageJson, /ghl:reconcile-workspaces/);
assert.match(packageJson, /ghl:review-provisioning-failures/);
...
assert.match(leadSyncProofRoute, /Click to Scale GHL lead-sync proof/);
assert.match(leadSyncProofRoute, /syncLeadToPartnerCrm/);
assert.match(leadSyncProofRoute, /idempotencyProven/);
assert.match(leadSyncProofRoute, /noSms: true/);
assert.match(leadSyncProofRoute, /noMeta: true/);
assert.doesNotMatch(leadSyncProofRoute, /return apiSuccess\([^]*token/i);
assert.match(mappingRepairRoute, /requirePlatformAdmin/);
assert.match(mappingRepairRoute, /assertSameOriginRequest/);
assert.match(mappingRepairRoute, /partner_configs/);
assert.match(mappingRepairRoute, /partner_ghl_config/);
assert.match(mappingRepairRoute, /workspace_ghl_mapping/);
assert.match(mappingRepairRoute, /workspace_partner_attribution/);
assert.match(mappingRepairRoute, /CLICKTOSCALE_GHL_PRIVATE_INTEGRATION/);
assert.match(mappingRepairRoute, /printedSecrets: false/);
assert.doesNotMatch(mappingRepairRoute, /return apiSuccess\([^]*token/i);
assert.match(readinessScript, /failedProvisioningSql/);
assert.match(readinessScript, /failedLeadSyncSql/);
assert.match(readinessScript, /mappingHealthSql/);
assert.match(readinessScript, /accepted_deferred/);
assert.match(reconcileScript, /workspace_partner_attribution/);
assert.match(reconcileScript, /workspace_ghl_mapping/);
assert.match(reconcileScript, /Apply mode queues internal provisioning jobs only/);
assert.match(reviewFailuresScript, /accepted_deferred/);
assert.match(reviewFailuresScript, /ghl_user_invite_api_upgrade_pending/);
assert.match(reviewFailuresScript, /does not have access to this feature/);
...
assert.doesNotMatch(runbook.match(/Required private integration scopes:[\s\S]*?Safe validation:/)?.[0] ?? "", /contacts\.readonly/);
assert.doesNotMatch(runbook.match(/Required private integration scopes:[\s\S]*?Safe validation:/)?.[0] ?? "", /opportunities\.readonly/);
assert.match(runbook, /Workflow Enrollment/);
assert.match(runbook, /Do not enable workflow enrollment/);
assert.match(runbook, /Backfill \/ Reconciliation/);
assert.match(runbook, /Review accepted provisioning failures/);
