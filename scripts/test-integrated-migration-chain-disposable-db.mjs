...
async function proveExact104ThenForward120Then121Then122Then123Then128() {
  return adapter.withDisposableDatabase(async (session) => {
    installRemoteEquivalentDefaults(session);
    ...
    const successor128 = applyMigrations(session, migrations.slice(123));
    assert.equal(successor128.applied.length, 5);
    assert.equal(successor128.skipped.length, 0);
    assert.equal(
      session.psql(
        "select count(*) || '|' || max(version) from supabase_migrations.schema_migrations;",
        { label: "Verify exact post-successor-128 migration history" },
      ),
      "128|20260727010000",
    );
    verifyIntegratedObjects(session);
    ...
    assert.equal(replay.applied.length, 0);
    assert.equal(replay.skipped.length, 24);
    assert.deepEqual(
      {
        schema: normalizedSchemaDump(session),
        catalog: normalizedManagedCatalog(session),
...
try {
  ...
  const forward104To128 = await proveExact104ThenForward120Then121Then122Then123Then128();
  await provePerFileAtomicFailure();
  assert.deepEqual(staged, fresh);
  assert.deepEqual(forward104To128, fresh);
  assert.deepEqual(adapter.listDisposableDatabases(), []);
  console.log(
    `Integrated migration chain PASS: ${migrations.length} migrations, ` +
      `fresh/replay/foundation-extension/forward-104-to-120-to-121-to-122-to-123-to-128 schema=${fresh.schema.digest}, ` +
