...
const operatorDebtService = read("src/lib/services/operator-debt-service.ts");
...
const operatorDebtRoute = read("src/app/api/admin/operator-debt/[id]/route.ts");
const controlRoomPage = read("src/app/(app)/admin/control-room/page.tsx");
...
const resolveKnownDebt = read("scripts/resolve-known-operator-debt.mjs");
...
assert.equal(
  packageJson.scripts["operator:ops-summary"],
  "node ./scripts/print-ops-monitoring-summary.mjs",
  "ops summary command must be registered",
);
...
assert.match(operatorDebtService, /loadOpenOperatorDebtJobs/, "operator debt service must expose open failed/dead-letter jobs");
assert.match(operatorDebtService, /acknowledgeOperatorDebtJob/, "operator debt service must expose acknowledgement action");
assert.match(operatorDebtService, /reviewed_at/, "operator debt acknowledgement must preserve reviewed_at audit trail");
assert.match(operatorDebtService, /resolution_note/, "operator debt acknowledgement must require a resolution note");
assert.match(operatorDebtService, /known_proof_residue/, "known proof residue must be classified explicitly");
assert.match(operatorDebtService, /column leads\.consent_source does not exist/, "known public QA billing debt must be narrowly matched");
assert.match(operatorDebtService, /retriedJob: false/, "operator debt acknowledgement must not retry jobs");
assert.match(operatorDebtService, /deletedJob: false/, "operator debt acknowledgement must not delete jobs");
assert.match(operatorDebtService, /externalMutation: false/, "operator debt acknowledgement must not trigger external mutations");
assert.doesNotMatch(operatorDebtService, /\.delete\(/, "operator debt service must not delete evidence rows");
assert.doesNotMatch(operatorDebtService, /runSystemJobWorkerBatch|processSystemJob|claimSystemJobByIdForWorker/, "operator debt service must not retry or process jobs");
assert.match(operatorDebtRoute, /assertSameOriginRequest/, "operator debt action route must require same-origin POSTs");
assert.match(operatorDebtRoute, /assertInternalOperatorAccess/, "operator debt action route must require operator access");
assert.match(operatorDebtRoute, /acknowledgeOperatorDebtJob/, "operator debt route must use the audit acknowledgement service");
assert.doesNotMatch(operatorDebtRoute, /runSystemJobWorkerBatch|processSystemJob|claimSystemJobByIdForWorker|delete\(/, "operator debt route must not retry or delete jobs");
assert.match(controlRoomPage, /loadOpenOperatorDebtJobs/, "control room must load operator debt jobs");
assert.match(controlRoomPage, /Operator debt V2/, "control room must show operator debt V2 section");
assert.match(controlRoomPage, /Acknowledge with audit note/, "control room must expose audit acknowledgement action");
assert.match(controlRoomPage, /never retries, deletes, sends SMS\/email, charges Stripe, calls GHL, mutates Meta, or runs providers/, "control room must state recovery safety boundary");
assert.match(routeSecurity, /\/api\/admin\/operator-debt\/\[id\]/, "route security must cover operator debt acknowledgement route");
assert.match(opsSummary, /OPS_SUMMARY_APP_URL/, "ops summary must allow explicit production alias override");
assert.match(opsSummary, /https:\/\/app\.agentdealflow\.io/, "ops summary must default to the production app alias");
assert.match(opsSummary, /operatorReviewedAt/, "ops summary must exclude reviewed provider and CRM proof residue");
assert.match(opsSummary, /sendsSlack: false/, "ops summary must not send Slack");
assert.match(opsSummary, /sendsEmail: false/, "ops summary must not send email");
assert.match(opsSummary, /mutatesGhl: false/, "ops summary must not mutate GHL");
assert.match(opsSummary, /mutatesMeta: false/, "ops summary must not mutate Meta");
assert.match(opsSummary, /createsStripeCharge: false/, "ops summary must not create Stripe charges");
assert.match(resolveKnownDebt, /KNOWN_GHL_OPPORTUNITY_V1_AUTH_DEBT_IDS/, "known resolver must include GHL opportunity proof residue");
assert.match(resolveKnownDebt, /operatorReviewedAt/, "known resolver must mark CRM proof residue in metadata");
assert.match(resolveKnownDebt, /KNOWN_HISTORICAL_CLIENT_ERROR_IDS/, "known resolver must include historical client error telemetry");
