...
function runOfflineChecks() {
  ...
  const leadCaptureStrategyService = "src/lib/services/lead-capture-strategy-service.ts";
  const leadFormTemplateService = "src/lib/services/lead-form-template-service.ts";
  const leadCaptureReadinessService = "src/lib/services/lead-capture-readiness-service.ts";
  const metaLeadFormService = "src/lib/services/meta-lead-form-service.ts";
  const leadCaptureMigration = "supabase/migrations/20260605210000_add_lead_capture_strategy.sql";
  ...
  assertIncludes(creativeChatIntake, "Requesting a revision will pause paid rendering until the revised brief is approved again", "Creative intake render boundary", "creative chat intake explains that paid render work waits for approval");
  ...
  assertIncludes(".env.example", "LEAD_CAPTURE_STRATEGY_ENABLED=false", "Lead capture strategy feature flag documented", "strategy defaults off until intentionally enabled");
  assertIncludes(".env.example", "META_INSTANT_FORMS_ENABLED=false", "Meta Instant Forms feature flag documented", "Instant Forms default off");
  assertIncludes(".env.example", "LEAD_FORM_LAUNCH_ENABLED=false", "Lead form launch feature flag documented", "live lead form launch default off");
  assertIncludes(leadCaptureStrategyService, "website_funnel", "Lead capture website funnel option", "legacy funnel path remains a first-class capture method");
  assertIncludes(leadCaptureStrategyService, "meta_instant_form", "Lead capture Meta Instant Form option", "Instant Forms can be represented without mutating Meta");
  assertIncludes(leadCaptureStrategyService, "HOUSING", "Lead capture housing category", "real estate lead capture defaults to Housing special ad category");
  assertIncludes(leadFormTemplateService, "volume_instant_form", "Lead form volume template", "low-friction template exists");
  assertIncludes(leadFormTemplateService, "balanced_instant_form", "Lead form balanced template", "qualification template exists");
  assertIncludes(leadFormTemplateService, "quality_website_funnel", "Lead form quality template", "website funnel quality template exists");
  assertIncludes(leadCaptureReadinessService, "meta_connection_missing", "Lead capture Meta readiness gate", "Instant Forms require Meta connection before launch");
  assertIncludes(leadCaptureReadinessService, "housing_special_ad_category_missing", "Lead capture housing readiness gate", "Instant Forms fail closed without Housing category");
  assertIncludes(metaLeadFormService, "assertLiveMetaLeadFormCreationAllowed", "Meta lead form live mutation guard", "live form creation is explicitly gated");
  assertIncludes(metaLeadFormService, "mockOnly", "Meta lead form mock proof mode", "tests can prove payload shape without mutating Meta");
  assertIncludes(leadCaptureMigration, "enable row level security", "Lead capture tables enable RLS", "new lead capture tables are RLS protected");
  assertIncludes(leadCaptureMigration, "force row level security", "Lead capture tables force RLS", "new lead capture tables force RLS");
  assertIncludes(leadCaptureMigration, "private.is_current_user_org_member", "Lead capture tenant member policy", "member reads are tenant scoped");
