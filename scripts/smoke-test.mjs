...
function runOfflineChecks() {
  ...
  const leadTrackingService = "src/lib/services/lead-tracking-service.ts";
  const browserPixelRoute = "src/app/api/lead-tracking/browser-pixel/route.ts";
  ...
  const uiDirectionPage = "src/app/ui-direction/page.tsx";
  ...
  const creativeIntakeLanguage = "src/lib/services/creative-intake-language.ts";
  ...
  const videoGenerationJob = "src/lib/services/video-generation-job.ts";
  ...
  assertOrderedIncludes(
    ...
    "guided onboarding splits audience, budget, setup, and offer so the form stays focused",
  );
  ...
  assertIncludes(onboardingPage, "Single Family Homes", "Buyer single-family inventory coverage", "buyer mode includes the combined detached/townhome/freestanding/large-lot inventory choice");
  assertIncludes(onboardingPage, "First Time Buyer Homes", "Buyer first-time inventory coverage", "buyer mode includes a first-time buyer inventory choice");
  assertIncludes(onboardingPage, "New Construction", "Buyer new-construction inventory coverage", "buyer mode includes new construction inventory");
  assertIncludes(onboardingPage, "Luxury Homes", "Buyer luxury inventory coverage", "buyer mode includes luxury inventory");
  assertIncludes(onboardingPage, "Multi Unit Homes", "Buyer multi-unit inventory coverage", "buyer mode includes multi-unit inventory");
  ...
  assertIncludes(prepaywallPreview, "CanonicalFunnelRenderer", "Pre-paywall funnel preview", "onboarding includes the canonical funnel preview shell");
  ...
  assertIncludes(prepaywallPreview, "lg:grid-cols-2", "Pre-paywall compact grid", "normal onboarding preview uses equal preview columns instead of an uneven ad/funnel split");
  assertIncludes(prepaywallPreview, "max-w-[560px]", "Pre-paywall centered copy angle", "copy angle is centered below both preview columns instead of nesting under only the funnel preview");
  assertIncludes(prepaywallPreview, "compact ? \"self-start p-3\" : \"p-4\"", "Pre-paywall compact ad geometry", "normal onboarding and paywall sidecar previews keep the ad mock from stretching vertically beside tall panels");
  assertIncludes(prepaywallPreview, "compact ? \"mt-7 h-[320px] max-h-[42vh]\" : \"mt-8 aspect-[4/3]\"", "Pre-paywall ad height guard", "compact ad preview content uses a bounded phone-style viewport instead of elongating");
  assertIncludes(prepaywallPreview, "xl:grid-cols-2", "Pre-paywall package grid", "review/paywall package preview uses equal preview columns before centering the copy preview below");
  ...
  assertIncludes(onboardingPage, 'fetch("/api/billing/checkout"', "Onboarding direct checkout handoff", "final step starts Pro checkout directly with the persisted campaign id");
  ...
  assertIncludes(onboardingPage, "Recommended audience", "Onboarding audience recommendation", "audience step recommends an audience instead of requiring agents to invent one");
  assertIncludes(onboardingPage, "Custom price range or deal size", "Onboarding custom deal size", "audience step supports a custom price range or deal size");
  assertIncludes(onboardingPage, "Recommended starting budget: $30-$50/day", "Onboarding budget guidance", "budget step explains the recommended starter media budget");
  assertIncludes(onboardingPage, "\"$10/day\"", "Onboarding $10 budget", "budget step supports a $10/day preset");
  assertIncludes(onboardingPage, "\"$20/day\"", "Onboarding $20 budget", "budget step supports a $20/day preset");
  assertIncludes(onboardingPage, "recommendLeadCaptureMode", "Onboarding lead-capture recommendation", "budget step auto-recommends capture style from daily spend");
  assertIncludes(onboardingPage, "Under $30/day: keep the form simple", "Onboarding instant form warning", "instant-form setup warns low-budget users not to add friction");
  assertIncludes(onboardingPage, "Selected {draft.leadFormQuestions.length}/3", "Onboarding instant form question cap", "instant-form setup caps custom lead form questions at three");
  assertIncludes(onboardingPage, "Risk reversals like free, no obligation", "Onboarding offer education", "offer step explains risk reversal and timeframe quality guidance");
  ...
  assertIncludes(buildFunnelPage, "buildOnboardingHref", "Funnel route setup redirect", "legacy funnel subroute sends users back to guided setup instead of the retired Builder workspace");
  assertIncludes(buildCreativesPage, "buildOnboardingHref", "Creatives recovery setup redirect", "creative prerequisites send users back to guided setup instead of the retired Builder workspace");
  ...
  assertIncludes(paywallPage, "Back to setup", "Paywall back path", "activation back controls return to guided setup when a campaign exists");
  assertIncludes(previewPage, "redirect(\"/onboarding\")", "Preview missing state redirect", "review recovers into guided setup instead of the retired Builder workspace");
  assertIncludes(previewPage, "Back to Creative Studio", "Preview back path", "review back controls return to Creative Studio");
  ...
  assertIncludes(creativeWizard, "Open launch creative", "Creative launch package preview controls", "creative selection shows each launch creative in an expandable side-by-side package");
  assertIncludes(creativeWizard, "Launch creative", "Creative launch package modal", "creative selection opens a focused creative preview dialog");
  assertExcludes(creativeWizard, "Creative carousel", "Creative carousel removed", "customers should not see a separate creative carousel chooser");
  assertExcludes(creativeWizard, "View all creatives and choose", "Creative chooser copy removed", "customers should not be asked to choose 3-6 creatives");
  assertExcludes(creativeWizard, "Click any card to view it large above", "Creative carousel inspection cue removed", "the launch package is automatic instead of carousel-driven");
  assertIncludes(creativeWizard, "Launch package", "Creative launch package summary", "creative selection shows the automatic launch package instead of a carousel");
  assertIncludes(creativeWizard, "DealFlow automatically keeps the launch-ready static ads selected", "Creative automatic selection copy", "creative selection explains that launch-ready assets are selected automatically");
  ...
  assertExcludes(creativeWizard, "snap-x", "Creative carousel layout removed", "creative selection no longer renders a carousel-style chooser");
  ...
  assertIncludes(creativeWizard, "Review the launch-ready creative set", "Creative simplified review heading", "creative review uses a cleaner heading without the old approval card stack");
  assertIncludes(creativeWizard, "DealFlow automatically keeps the launch-ready static ads selected", "Creative automatic launch package guidance", "creative review explains automatic selection in the main header");
  assertExcludes(creativeWizard, "Approved brief source: saved creative intake", "Creative approved-source callout removed", "creative review no longer shows the green approved-source callout");
  assertIncludes(creativeWizard, "Save launch package and continue", "Creative package continue CTA", "creative selection has one clear save-and-continue action after reviewing the launch package");
  assertExcludes(creativeWizard, "Selected creative preview", "Creative oversized single preview removed", "creative selection no longer leads with one oversized preview card");
  assertExcludes(creativeWizard, "Primary creative", "Creative primary label removed", "creative selection no longer labels one creative as primary in the customer review grid");
  assertExcludes(creativeWizard, "Review variant", "Creative review variant label removed", "creative selection no longer uses review-variant labels in the customer review grid");
  ...
  assertIncludes(creativeWizard, "Back to setup", "Creative wizard setup return", "creative selection returns to guided setup instead of another setup flow");
  ...
  assertIncludes("src/components/funnel/funnel-preview.tsx", "CanonicalFunnelRenderer", "Funnel preview canonical renderer", "operator previews use the shared canonical funnel renderer");
  assertIncludes("src/components/funnels/canonical-funnel-renderer.tsx", "max-w-[920px]", "Public funnel centered reference layout", "public funnels use the compact centered reference opt-in structure");
  assertIncludes("src/components/funnels/canonical-funnel-renderer.tsx", "id=\"lead-capture\"", "Public funnel lead-capture anchor", "public funnels expose an above-fold CTA anchor to the lead form");
  assertIncludes("src/components/funnels/canonical-funnel-renderer.tsx", "ReferenceOptInPreviewCard", "Public funnel opt-in card", "public funnel preview uses the simple name/email/phone opt-in card");
  assertIncludes(leadForm, "consent:", "Public funnel compliance consent", "lead capture keeps explicit localized SMS consent copy");
  assertIncludes(leadForm, "sms_consent_copy: copy.consent", "Public funnel compliance consent payload", "lead capture submits the active localized SMS consent copy");
  ...
  assertIncludes("src/lib/funnels/canonical-funnel.ts", "containsLegacyDefaultFunnelCopy", "Funnel preview offer override", "existing saved funnels are detected and rebuilt when stored copy is stale");
  assertExcludes("src/components/funnels/canonical-funnel-renderer.tsx", "RenderSection", "Funnel preview duplicate section guard", "review preview cannot repeat stale long-form sections below the canonical hero");
  ...
  assertIncludes(buildCreativesPage, "Review the structured creative brief before paid image or video renders are prepared", "Creative chat intake UI", "guided intake collects creative direction before paid image or video rendering");
  assertIncludes(creativeChatIntake, "CREATIVE_INTAKE_LOCKED_STATIC_STYLE_LABEL", "Creative intake locked static direction", "creative intake uses a single bold offer-focused static direction");
  assertExcludes(creativeChatIntake, "Choose static ad direction", "Creative intake static choice removed", "creative intake no longer asks customers to choose static ad direction");
  ...
  assertExcludes(creativeChatIntake, "Creative brief approved", "Creative compact approval banner removed", "approved creative intake does not add an extra card above the review workspace");
  assertExcludes(creativeChatIntake, "Requesting a revision will pause paid rendering until the revised brief is approved again", "Creative revision note copy removed", "the simplified review workspace avoids the old revision-note block");
  assertExcludes(creativeChatIntake, "Open Marketing Studio chat", "Creative intake Marketing Studio surface moved", "Marketing Studio opens from the review workspace controls instead of the removed compact approval card");
  assertIncludes(creativeWizard, "Open Marketing Studio chat", "Creative review Marketing Studio surface", "creative review exposes Marketing Studio beside the Static Ads and UGC Videos controls");
  ...
  assertIncludes(creativeChatIntake, "Creative language", "Creative intake language UI", "creative intake lets customers choose the creative language before provider generation");
  assertIncludes(creativeIntakeLanguage, "Do not mix in English", "Creative language contract", "French and Spanish creative prompts explicitly prevent English language mixing");
  assertIncludes(creativeChatIntakeService, "languageInstruction", "Creative prompt language wiring", "creative prompt builder carries the selected creative language into provider instructions");
  assertIncludes(staticCreativeAssetService, "targetLanguage", "Static creative language metadata", "static creative assets persist the selected creative language in generation metadata");
  assertIncludes(videoGenerationJob, "targetLanguage", "Video creative language metadata", "video creative jobs persist the selected creative language in generation metadata");
  ...
  assertIncludes(previewPage, "2xl:grid-cols-3", "Preview balanced creative grid", "preview page keeps selected static creatives side by side instead of stacking one primary creative above variants");
  assertExcludes(previewPage, "Primary creative", "Preview no primary creative hierarchy", "preview page treats the selected static creatives as equal launch-test assets");
  assertExcludes(previewPage, "Review variants", "Preview no stacked review variants", "preview page no longer creates a tall vertical variant section");
  assertExcludes(previewPage, "UGC video options", "Preview no unselected UGC fallback", "preview page does not show UGC video placeholders when the customer skipped UGC");
  ...
  assertIncludes(builderPage, "getCanonicalBuildDestination", "Builder redirect resolver", "builder redirects customer traffic into the current guided campaign destination");
  ...
  assertIncludes(uiDirectionPage, "redirect(\"/onboarding\")", "UI direction demo route retired", "/ui-direction redirects into current onboarding instead of rendering old mockups");
  assertExcludes(uiDirectionPage, "DealFlow OS UI mockups", "UI direction mockup copy removed", "/ui-direction no longer exposes stale demo copy");
  ...
  assertIncludes(settingsPage, "Launch access active", "QA billing acceptance settings copy", "settings shows launch access without exposing internal override language");
  assertExcludes(settingsPage, "Internal billing override active", "Settings internal billing override hidden", "settings does not expose internal billing override language to customers");
  assertExcludes(settingsPage, "Owner/test billing accepted", "Settings owner/test billing copy hidden", "settings does not expose owner/test proof wording to customers");
  ...
  assertIncludes(launchPage, "Paused setup ready", "Launch paused setup tracking warning", "partial tracking keeps paused object creation visible while blocking live activation");
  ...
  assertIncludes(campaignPublishPanel, "What would you like your public slug to be?", "Publish panel slug-first copy", "public funnel publishing asks only for the customer-facing slug");
  assertIncludes(campaignPublishPanel, "Publish live", "Publish panel single action", "public funnel publishing exposes one customer-facing publish action");
  assertExcludes(campaignPublishPanel, "Keep as Draft", "Publish panel hides draft action", "customers are not asked to choose draft state from the publish panel");
  assertExcludes(campaignPublishPanel, "Stage Snapshot", "Publish panel hides staging action", "customers are not shown staging implementation details");
  assertExcludes(campaignPublishPanel, "Stage or publish the public funnel", "Publish panel hides staging headline", "public funnel publishing is framed as one guided live-publish action");
  ...
  assertExcludes(creativeWizard, "Full-resolution creative files stay inside this workspace", "Creative no-download policy callout removed", "creative review no longer shows the extra full-resolution file callout");
  ...
  assertExcludes(staticCreativePreviewCard, "Full-resolution creative files stay inside this workspace and are used through the launch workflow", "Creative direct-file copy removed", "customer preview cards stay focused on creative content instead of file-storage copy");
  ...
  assertIncludes(metaPayloadGuardrails, ".normalize(\"NFD\")", "Meta French market normalization", "Meta launch normalizes accented French market names before special-ad country inference");
  assertIncludes(metaPayloadGuardrails, "lanaudiere", "Meta Quebec region country inference", "Meta launch maps Martine's Lanaudiere market to Canada for housing special-ad category country");
  assertIncludes(launchRoute, "ensureMetaCampaignHousingSettings", "Meta campaign housing retry repair", "Meta launch reasserts housing special-ad category country before retrying ad set creation");
  ...
  assertExcludes(metaPayloadGuardrails, "degrees_of_freedom_spec", "Meta deprecated creative spec blocked", "Meta creative payloads do not send deprecated degrees_of_freedom_spec");
  assertExcludes(metaPayloadGuardrails, "standard_enhancements", "Meta deprecated creative enhancement blocked", "Meta creative payloads do not send deprecated standard_enhancements");
  assertExcludes(launchRoute, "degrees_of_freedom_spec", "Direct launch deprecated creative spec blocked", "direct Meta launch creative creation avoids deprecated degrees_of_freedom_spec");
  ...
  assertIncludes(metaConnect, "sanitizeMetaReturnPath", "Meta OAuth return path guard", "connect route sanitizes return paths through the shared current-route allowlist");
  ...
  assertIncludes(metaCallback, "sanitizeMetaReturnPath", "Meta OAuth callback origin guard", "callback redirects through the shared current-route allowlist");
  ...
  assertIncludes(leadForm, "Please provide your email", "Lead form email validation", "lead form requires email before submit");
  assertIncludes(leadForm, "Please provide your phone number", "Lead form phone validation", "lead form requires phone before submit");
  ...
  assertExcludes(leadRoute, "TURNSTILE_SECRET_KEY", "Lead capture no Turnstile server gate", "public customer funnels do not depend on Cloudflare Turnstile");
  assertExcludes(leadRoute, "turnstile_required", "Lead capture no missing-token rejection", "public customer funnels cannot be blocked by a missing Turnstile token");
  assertExcludes(leadRoute, "ALLOW_PUBLIC_LEAD_NO_TURNSTILE", "Lead capture no bypass flag", "public lead capture has no Turnstile bypass flag because Turnstile is not part of the funnel flow");
  ...
  assertExcludes(leadRoute, "https://challenges.cloudflare.com/turnstile/v0/siteverify", "Lead capture no Turnstile siteverify", "public lead capture does not call Cloudflare Turnstile");
  assertExcludes(leadForm, "NEXT_PUBLIC_TURNSTILE_SITE_KEY", "Lead form no Turnstile client gate", "public lead forms never render customer-facing Turnstile");
  ...
  assertIncludes(leadForm, "getCurrentPageAttribution", "Lead form attribution capture", "public lead form sends current page attribution instead of relying only on Referer");
  assertIncludes(leadForm, "await waitForMetaPixelDispatch()", "Lead form Meta pixel dispatch window", "public lead form gives the browser Lead pixel a short dispatch window before thank-you navigation");
  assertIncludes(leadForm, "recordBrowserPixelAttempt", "Lead form browser pixel ledger", "public lead form records browser pixel attempts separately from CAPI");
  assertIncludes(browserPixelRoute, "assertSameOriginRequest", "Browser pixel telemetry CSRF guard", "browser pixel telemetry is same-origin guarded");
  assertIncludes(browserPixelRoute, "eventType: \"browser_pixel_attempted\"", "Browser pixel telemetry ledger", "browser pixel telemetry writes a tracking event after validating the lead");
  assertIncludes(leadRoute, "parseLandingPageAttribution", "Lead capture attribution fallback", "lead capture backfills UTM/ad attribution from the landing URL");
  assertIncludes(leadRoute, "url.searchParams.get(\"utm_content\")", "Lead capture Meta ad attribution", "lead capture treats Meta utm_content as ad id attribution");
  assertIncludes(leadRoute, "eventType: \"lead_captured\"", "Lead captured tracking ledger", "public lead capture writes durable lead tracking events");
  assertIncludes(leadRoute, "eventType: \"capi_queued\"", "Lead CAPI queued tracking ledger", "public lead capture records queued CAPI work");
  assertIncludes(leadTrackingService, "campaign_tracking_contracts", "Campaign tracking contract service", "launches have a durable tracking contract service");
  assertIncludes(leadTrackingService, "lead_tracking_events", "Lead tracking event service", "lead tracking events have a durable service wrapper");
  assertIncludes(launchRoute, "tracking_contract_incomplete", "Launch tracking contract fail-closed", "Meta launch cannot complete with an incomplete tracking contract");
  ...
  assertIncludes(middleware, "https://www.facebook.com", "Meta pixel CSP allowlist", "production CSP allows the configured Facebook pixel beacon without broad image wildcards");
  assertIncludes(middleware, "https://capig.datah04.com", "Meta CAPI CSP allowlist", "production CSP allows the specific Meta CAPI gateway observed from the configured pixel without broad connect wildcards");
  ...
  assertIncludes(planPresentation, "SELECTABLE_PLAN_TIERS = [\"pro\"]", "Single public plan selector", "new onboarding and paywall plan selection only exposes Pro");
  assertIncludes(paywallPage, "Activate Operator Launch", "Paywall single-plan heading", "paywall no longer asks customers to choose between archived plans");
  assertIncludes(onboardingPage, "Confirm launch plan", "Onboarding single-plan step", "onboarding no longer labels the plan step as behavior selection");
  ...
  assertIncludes(launchRoute, "organizationId: credentials.workspaceId", "Launch campaign-scoped preflight", "direct Meta launch validates preflight against the same campaign organization used for credentials");
  ...
  assertExcludes(launchPage, "Owner/test billing acceptance is active for this campaign", "Launch owner/test billing banner hidden", "launch page does not expose owner/test billing proof language");
  assertExcludes(launchPage, "internal billing override", "Launch internal billing banner hidden", "launch page does not expose internal billing override language");
  ...
  assertIncludes(unlockPage, "Back to setup", "Unlock cancelled CTA", "cancelled checkout sends the user back to guided setup instead of dashboard by default");
  ...
  assertIncludes(builderPage, "buildCreativeStudioHref", "Builder creative CTA target", "Builder sends unselected campaigns to the creative workspace instead of refreshing itself");
