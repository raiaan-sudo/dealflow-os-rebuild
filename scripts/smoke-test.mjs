...
function runOfflineChecks() {
  ...
  const leadTurnstile = "src/lib/security/turnstile.ts";
  ...
  const metaContract = "src/lib/integrations/meta/contract.ts";
  ...
  assertIncludes(metaConnect, "resolveMetaReturnUrl", "Meta OAuth return path guard wiring", "OAuth start delegates return-path validation to the central Meta contract");
  assertIncludes(metaContract, "returnTo.startsWith(\"//\")", "Meta OAuth return path guard", "protocol-relative return paths are rejected by the central contract");
  assertIncludes(metaCallback, "resolveMetaReturnUrl", "Meta OAuth callback origin guard wiring", "callback redirects delegate to the central Meta contract");
  assertIncludes(metaContract, "resolved.origin === appOrigin", "Meta OAuth callback origin guard", "central contract keeps callback redirects on the app origin");
  ...
  assertIncludes(leadRoute, "verifyLeadCaptureTurnstile", "Lead capture Turnstile enforcement", "production lead capture verifies every non-test token before persistence");
  assertIncludes(leadTurnstile, "https://challenges.cloudflare.com/turnstile/v0/siteverify", "Lead capture Turnstile siteverify", "server validation uses Cloudflare Siteverify");
  ...
  assertIncludes(twilioWebhookRoute, "twilio:webhook:authenticated-destination", "Twilio authenticated destination rate limit", "valid signed webhooks use a durable account-and-destination bucket after signature verification");
  ...
  assertIncludes(leadRoute, "LEAD_CAPTURE_LOAD_TEST_BYPASS_ENABLED", "Lead load proof gate", "synthetic no-write load proof requires an explicit env gate");
  assertIncludes(leadRoute, "x-dealflow-load-test-secret", "Lead load proof secret", "synthetic no-write load proof requires a server-side secret header");
  assertIncludes(leadRoute, "synthetic-no-write", "Lead load proof no-write contract", "the load-only path returns before any lead or provider-effect write");
  ...
  assertIncludes(systemJobService, "claim_next_system_job_v2", "Atomic system job claim", "system job worker uses the versioned DB-backed SKIP LOCKED claim protocol");
