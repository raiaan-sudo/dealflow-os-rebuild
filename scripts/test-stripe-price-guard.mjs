...
assert.match(presentation, /SELECTABLE_PLAN_TIERS = \["pro"\]/, "Archived Performance and Starter plans must not be customer-selectable");
assert.match(presentation, /performance:[\s\S]*checkoutCtaLabel:\s*"Start Performance checkout"/, "Performance must have no free-trial checkout copy when referenced internally");
assert.match(presentation, /checkoutCtaLabel:\s*"Get started now"/, "New public checkout copy must use the single $297 plan CTA");
