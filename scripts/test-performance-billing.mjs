...
assert.match(presentation, /SELECTABLE_PLAN_TIERS = \["pro"\]/, "New customer-facing plan selection must be Pro-only");
assert.match(presentation, /checkoutCtaLabel:\s*"Get started now"/, "Public checkout CTA must use the single-plan copy");
assert.match(presentation, /Only launch plan/, "Pro must be positioned as the only launch plan for new users");
assert.match(presentation, /performance:[\s\S]*checkoutCtaLabel:\s*"Start Performance checkout"/, "Archived Performance checkout copy must remain available internally");
...
assert.match(performanceService, /getLeadConsentSource/);
assert.match(performanceService, /\.select\("id,organization_id,campaign_id,source,consent_metadata,created_at,metadata"\)/);
assert.doesNotMatch(performanceService, /\.select\("[^"]*consent_source[^"]*"\)/, "Performance billing must not query a non-existent leads.consent_source column");
assert.doesNotMatch(performanceService, /params\.lead\.consent_source/, "Performance billing must read consent source from leads.consent_metadata.source");
