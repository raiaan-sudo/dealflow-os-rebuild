...
const forward120To121Contract = readFileSync(
...
const forward122To123Contract = readFileSync(
  join(root, "scripts", "staging", "forward-122-to-123-contract.mjs"),
  "utf8",
);
...
assert.ok(browserContextBoundaryTest.includes(["challenges", "cloudflare", "com"].join(".")));
...
assert.match(runner, /parsed\.exactSyntheticAuthUserCount !== 11 \+ expectedPreservedGhlOwnerCount/);
assert.match(runner, /PRESERVED_ZERO_SPEND_SYNTHETIC_GHL_AUTHORITY/);
assert.match(runner, /parsed\.exactPreservedGhlOwnerAuthUserCount !== expectedPreservedGhlOwnerCount/);
assert.match(runner, /ghl_marketplace_encrypted_credentials === 4/);
assert.match(runner, /parsed\.qaHarness\?\.role !== "member"/);
assert.match(runner, /parsed\.qaHarness\?\.authorityResetPolicyApplied !== true/);
assert.match(runner, /parsed\.qaHarness\?\.organizationMembershipCount !== 1/);
assert.match(runner, /parsed\.qaHarness\?\.partnerMembershipCount !== 0/);
...
assert.match(
  ...
  "the common final gate must accept the exact 122-to-123 successor status",
);
...
assert.doesNotMatch(
  ...
  "current-123 resume must validate its own retained prior application instead of the historical 104 transition authority",
);
...
assert.match(forward122To123Contract, /migrationCount: 123/);
assert.match(forward122To123Contract, /20260722020000_persist_ghl_location_token_scope\.sql/);
...
assert.match(runner, /const exactSyntheticDeletionReset/);
assert.match(runner, /"exact_prior_fixture_removed"/);
assert.match(runner, /syntheticDeletionReset\?\.exactFixtureOnly === true/);
assert.match(runner, /syntheticDeletionReset\.postResetDeletionRequestCount === 0/);
assert.match(runner, /syntheticDeletionReset\.postResetSuspensionCount === 0/);
assert.match(runner, /syntheticDeletionReset\.postResetPrivacyRequestCount === 0/);
assert.match(runner, /syntheticDeletionReset\.customerDataAccessed === false/);
...
assert.ok(browserSessionContract.includes(["challenges", "cloudflare", "com"].join(".")));
...
assert.match(browserSpec, /WebKitBlobResource error 1/);
assert.match(browserSpec, /await readResponse\.finished\(\)/);
assert.doesNotMatch(browserSpec, /networkidle/);
...
assert.match(browserSpec, /url\.pathname === "\/api\/auth\/session"/);
assert.match(browserSpec, /testTitle ===\s*"new direct realtor is authenticated but remains unpaid and launch-blocked"/);
assert.match(browserSpec, /Object\.keys\(payload\)\.sort\(\)\.join\(","\) === "action,email,password"/);
assert.match(browserSpec, /payload\.action === "sign-in"/);
assert.match(browserSpec, /payload\.email === ROLE_EMAILS\.newDirect/);
assert.match(browserSpec, /payload\.password === requiredEnvironment\("STAGING_QA_PASSWORD"\)/);
assert.match(browserSpec, /!isExactServerPasswordSignIn/);
...
assert.ok(safeBrowserHostTest.includes(["evil", "example", "com"].join(".")));
...
assert.match(runner, /parsed\.accountDeletion\?\.protectedLowLevelFunctionDirectlyCallable !== false/);
assert.match(runner, /parsed\.accountDeletion\?\.privacyAuthorityWrapperUsed !== true/);
...
assert.match(providerIndependentProof, /create_privacy_delete_request_v1/);
assert.match(providerIndependentProof, /Protected low-level account deletion function was directly callable/);
assert.match(providerIndependentProof, /privacyAuthorityWrapperUsed: true/);
...
assert.match(successorProviderIndependentContract, /SUCCESSOR_SCHEMA_VERSION = "20260720010000"/);
...
assert.match(
  help.stdout,
  /Historical bounded forward transition from the pinned read-only-proven[\s\S]*\n  node[^\n]* \\\n    --execute --apply-forward-migration --deploy \\\n    --prior-migration-proof-dir \/absolute\/path\/pinned-104\/migration-proof/s,
  "historical forward-mode help must preserve executable multiline shell continuations",
);
assert.match(
  help.stdout,
