...
assert.match(source, /tracked_or_unignored_supabase_cli_state/);
assert.match(source, /path\.startsWith\("supabase\/\.temp\/"\)/);
assert.match(source, /readSecureFileSnapshot\(path, \{ maxBytes: MAX_TEXT_BYTES \}\)/);
assert.match(source, /secure_file_snapshot_not_regular/);
assert.match(source, /secure_file_snapshot_too_large/);
assert.doesNotMatch(source, /existsSync|lstatSync|readFileSync\(path\)/);
