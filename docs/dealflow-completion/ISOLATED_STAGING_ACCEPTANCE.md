...
5. Deploy the exact commit to the isolated Vercel staging project and verify deployment metadata. Assign each allowlisted app alias one at a time, prove the exact control-plane mapping, then run a credential-free, monotonic, 180-second edge-propagation gate. Only Vercel's exact `404 DEPLOYMENT_NOT_FOUND` surface may be retried; success requires DealFlow's exact closed 404 gate. Recheck the candidate mapping before loading the staging secret, then prove the unauthenticated, header, and cookie surfaces sequentially before assigning the next alias. Redirects, URL changes, public 200s, generic failures, mapping drift, and deadline exhaustion fail closed and trigger rollback.
