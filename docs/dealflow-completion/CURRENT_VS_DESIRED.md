...
|---|---|---|---|
| Provenance | Core domains map to Vercel deployment `dpl_J4Ksu4n7sjwdRv8tHBTa5ARDKg9E` at the baseline SHA | Isolated implementation commit `8194caffd2817792be99692952e0cad030f2241a`, tree `b90e934b36765f2ab167b0d9925e94cdfcc8dfbf`, descends from the baseline; the documentation seal is a docs-only descendant recorded by the external bundle manifest | Clean ancestry, hashes, two exact-seal verification rounds, and signed production evidence bundle |
...
| Migration | The original tracked chain assumed `public.campaign_plans` already existed and failed fresh replay | Recovered authority now drives an 80-migration forward portfolio; 14/14 local schema gates pass on PostgreSQL 17.6, including fresh/current/May-2/idempotent replay, RLS/private proof, fail-closed collision checks, mixed-version safety, deterministic replay, cleanup, and forward recovery | Separately authorized staging replay plus signed exact-deployment environment and zero-old-worker drain; no production migration is authorized |
