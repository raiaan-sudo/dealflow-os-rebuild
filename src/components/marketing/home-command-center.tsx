...
const signupHref = `${appBaseUrl}/onboarding?new=1`;
...
function OperatingLayerSection() {
  const breakdown = [
    {
      label: "01",
      title: "Promises get removed.",
      body: "No guarantee language, rented dashboards, fake proof, or call-first sales loop. The page moves straight into software access.",
      icon: Megaphone,
    },
    {
      label: "02",
      title: "The system gets installed.",
      body: "Funnel, creative direction, lead capture, routing, launch checks, dashboard state, and optimization logic live in one operating layer.",
      icon: CircuitBoard,
    },
    {
      label: "03",
      title: "Humans review the risky parts.",
      body: "AI coordinates the build. The DealFlow team oversees quality, strategy, and launch readiness before risky actions move.",
      icon: ShieldCheck,
    },
  ];

  return (
    <section className="border-y border-white/10 bg-[#020611] py-20 sm:py-28">
      <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 sm:px-6 lg:grid-cols-[0.86fr_1.14fr] lg:items-center lg:px-8">
        <Reveal>
          <p className="text-xs font-semibold uppercase text-cyan-200/80">Operating layer</p>
          <h2 className="mt-4 text-3xl font-semibold leading-tight text-white sm:text-5xl">
            The agency pitch gets replaced by a real acquisition workflow.
          </h2>
          <p className="mt-5 text-base leading-8 text-white/62">
            Real estate operators do not need another animated promise stack. They need to see what DealFlow actually
            controls: creative direction, funnel assembly, lead capture, routing, launch checks, reporting, optimization
            logic, and human oversight in one software-first flow.
          </p>
        </Reveal>

        <Reveal delay={120}>
          <div className="df-ambient-panel overflow-hidden rounded-2xl border border-indigo-200/15 bg-[#07101c] p-5 shadow-[0_44px_130px_-76px_rgba(129,140,248,0.9)] sm:p-6">
                ...
                return (
                  <article key={item.title} className="rounded-lg border border-white/10 bg-white/[0.035] p-5">
                    <div className="flex items-center justify-between gap-3">
                      <span className="font-mono text-xs text-cyan-200">{item.label}</span>
                      <span className="grid size-9 place-items-center rounded-lg border border-cyan-200/20 bg-cyan-200/10 text-cyan-100">
