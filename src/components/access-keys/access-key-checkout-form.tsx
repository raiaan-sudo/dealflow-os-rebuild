...
export function AccessKeyCheckoutForm({
  ...
  const [planTier, setPlanTier] = useState<SelectablePlanTier>("pro");
  const [buyerEmail, setBuyerEmail] = useState("");
  const [buyerName, setBuyerName] = useState("");
  const [error, setError] = useState<string | null>(null);
  const [isPending, setIsPending] = useState(false);
  const selectedPlan = getPlanPresentation(planTier);
        ...
        {SELECTABLE_PLAN_TIERS.map((tier) => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-3 text-xs leading-5 text-white/58">{plan.footer}</p>
            </button>
          );
        })}
      </div>
