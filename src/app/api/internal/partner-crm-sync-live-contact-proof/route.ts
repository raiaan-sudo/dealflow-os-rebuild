...
const WORKFLOW_V1_PROOF_RUN_ID = "ghl_workflow_v1_20260618_01";
const QA_EMAIL = "qa+ghl-live-sync-20260618-01@example.com";
const QA_NAME = "QA GHL Live Sync 20260618 01";
const WORKFLOW_QA_EMAIL = "qa+ghl-workflow-v1-20260618@example.com";
const WORKFLOW_QA_NAME = "QA GHL Workflow V1 20260618";

const bodySchema = z.object({
  proofRunId: z.enum([DEFAULT_PROOF_RUN_ID, OPPORTUNITY_V1_PROOF_RUN_ID, WORKFLOW_V1_PROOF_RUN_ID]).default(DEFAULT_PROOF_RUN_ID),
  leadId: z.string().uuid().optional(),
  mode: z.enum(["fullSync", "workflowRetry"]).default("fullSync"),
}).strict();
...
function assertWriteGates(proofRunId: string, mode: z.infer<typeof bodySchema>["mode"]) {
  if (mode === "workflowRetry") {
    if (proofRunId !== WORKFLOW_V1_PROOF_RUN_ID) {
      throw new ApiError(409, "Workflow retry mode is only allowed for the approved workflow proof.", "workflow_retry_proof_mismatch");
    }

    if (process.env.GHL_WORKFLOW_ENROLLMENT_ENABLED !== "true") {
      throw new ApiError(409, "GHL workflow enrollment is not enabled for this proof.", "ghl_workflow_enrollment_disabled");
    }

    if (process.env.GHL_CONTACT_WRITES_ENABLED === "true") {
      throw new ApiError(409, "GHL contact writes must remain disabled for workflow retry proof.", "ghl_contact_writes_enabled");
    }

    if (process.env.GHL_OPPORTUNITY_WRITES_ENABLED === "true") {
      throw new ApiError(409, "GHL opportunity writes must remain disabled for workflow retry proof.", "ghl_opportunity_writes_enabled");
    }
  } else if (process.env.GHL_CONTACT_WRITES_ENABLED !== "true") {
    throw new ApiError(409, "GHL contact writes are not enabled for this proof.", "ghl_contact_writes_disabled");
  }

  if (mode !== "workflowRetry" && proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID && process.env.GHL_OPPORTUNITY_WRITES_ENABLED !== "true") {
  ...
  if (proofRunId === WORKFLOW_V1_PROOF_RUN_ID) {
    if (mode !== "workflowRetry" && process.env.GHL_OPPORTUNITY_WRITES_ENABLED !== "true") {
      throw new ApiError(409, "GHL opportunity writes are not enabled for this proof.", "ghl_opportunity_writes_disabled");
    }
  ...
  } else if (process.env.GHL_WORKFLOW_ENROLLMENT_ENABLED === "true") {
...
async function loadExistingQaLead(admin: AdminClient, body: z.infer<typeof bodySchema>) {
  if (body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID) {
    const existing = await loadWorkflowProofLead(admin);
    if (existing) {
      return existing;
    }

    return await createWorkflowProofLead(admin);
  }
...
async function loadWorkflowProofLead(admin: AdminClient) {
  const { data, error } = await db(admin)
    .from("leads")
    .select("id, organization_id, campaign_id, source, email, first_name, last_name, name, created_at")
    .eq("organization_id", TARGET_WORKSPACE_ID)
    .eq("email", WORKFLOW_QA_EMAIL)
    .eq("source", "ghl_workflow_v1_proof")
    .maybeSingle();

  if (error) {
    throw new ApiError(500, error.message, "workflow_qa_lead_lookup_failed");
  }
...
async function createWorkflowProofLead(admin: AdminClient) {
  const campaign = await loadCampaignForProof(admin);
  ...
  const lead = {
    ...
    first_name: "QA",
    last_name: "GHL Workflow V1 20260618",
    name: WORKFLOW_QA_NAME,
    email: WORKFLOW_QA_EMAIL,
    phone: null,
    source: "ghl_workflow_v1_proof",
    status: "new",
    estimated_value: 0,
    notes: "Controlled internal GHL workflow enrollment proof. Do not contact.",
    campaign_name: campaign.public_slug ?? "ghl-workflow-v1-proof",
    lead_type: "qa_workflow_enrollment",
    created_at: now,
    updated_at: now,
    metadata: {
      proof_run_id: WORKFLOW_V1_PROOF_RUN_ID,
      proof_type: "ghl_workflow_enrollment_v1",
    ...
    consent_metadata: {
      proof_run_id: WORKFLOW_V1_PROOF_RUN_ID,
      source: "internal_partner_crm_sync_live_contact_proof",
      sms: {
        consented: false,
      },
    },
  ...
  if (error) {
    throw new ApiError(500, error.message, "workflow_qa_lead_insert_failed");
  }
...
async function loadWorkflowProofEvent(admin: AdminClient, proofRunId: string) {
  const { data, error } = await db(admin)
    .from("lead_crm_sync_events")
    .select("id, status, attempt_count, ghl_contact_id, ghl_opportunity_id, metadata, updated_at")
    .contains("metadata", { proof_run_id: proofRunId })
  ...
  if (error) {
    throw new ApiError(500, error.message, "workflow_proof_event_lookup_failed");
  }
  ...
  if (!event?.id || event.status !== "synced") {
    throw new ApiError(409, "The workflow proof CRM event must exist and be synced before workflow retry.", "workflow_proof_event_not_synced");
  }

  if (!event.ghl_contact_id) {
    throw new ApiError(409, "The workflow proof CRM event is missing a GHL contact ID.", "workflow_proof_contact_missing");
  }
  ...
  return {
    ...
    ghlOpportunityId: event.ghl_opportunity_id ?? null,
    metadata: event.metadata && typeof event.metadata === "object" ? event.metadata : {},
    updatedAt: event.updated_at ?? null,
  };
}

async function loadWorkflowConfig(admin: AdminClient) {
  const { data, error } = await db(admin)
  ...
  if (error) {
    throw new ApiError(500, error.message, "workflow_config_lookup_failed");
  }

  if (!data?.enabled || typeof data.workflow_id !== "string" || !data.workflow_id.trim()) {
    throw new ApiError(409, "The approved partner workflow config is missing or disabled.", "workflow_config_missing");
  }

  return {
    workflowId: data.workflow_id.trim(),
    enrollmentTrigger: typeof data.enrollment_trigger === "string" ? data.enrollment_trigger : null,
  };
}

async function runWorkflowRetryProof(admin: AdminClient) {
  ...
  const workflowConfig = await loadWorkflowConfig(admin);
  const event = await loadWorkflowProofEvent(admin, WORKFLOW_V1_PROOF_RUN_ID);
  const token = getGhlPrivateTokenFromCredentialRef(config.credentialRef);

  if (!token) {
    throw new ApiError(409, "The approved GHL credential is not configured.", "ghl_auth_missing");
  }

  const beforeCounts = await countRows(admin);
  const enrollment = await new GoHighLevelClient({ token }).addContactToWorkflow({
  ...
  const nextMetadata = {
    ...event.metadata,
    workflow_retry_after_publish: true,
    workflow_retry_proof_run_id: WORKFLOW_V1_PROOF_RUN_ID,
    workflow_retry_at: now,
    workflow_retry_reason: "published_workflow_ui_verification",
    workflow_retry_success: enrollment.success,
    workflow_retry_message: enrollment.message,
    workflow_retry_enrollment_id: enrollment.enrollmentId,
  };
  ...
  if (updateError) {
    throw new ApiError(500, updateError.message, "workflow_retry_event_update_failed");
  }

  const afterCounts = await countRows(admin);

  return apiSuccess({
    success: Boolean(enrollment.success),
    proof: "partner_crm_workflow_retry_after_publish",
    proofRunId: WORKFLOW_V1_PROOF_RUN_ID,
    mode: "workflowRetry",
    target: {
      workspaceId: TARGET_WORKSPACE_ID,
      partnerId: TARGET_PARTNER_ID,
      ...
      workflowId: workflowConfig.workflowId,
      crmEventId: event.id,
      contactIdMasked: maskExternalId(event.ghlContactId),
      opportunityIdMasked: maskExternalId(event.ghlOpportunityId),
    },
    resolved: {
      mappingResolved: true,
      configResolved: true,
      workflowConfigResolved: true,
      credentialConfigured: true,
      enrollmentTrigger: workflowConfig.enrollmentTrigger,
    },
    writeGateStatus: {
    ...
    workflowEnrollmentResult: {
      ...
      workflowSuccess: enrollment.success,
    },
    ...
    safety: {
      ...
      workflowEnrollmentGate: "GHL_WORKFLOW_ENROLLMENT_ENABLED",
      contactWriteGateRequired: false,
      opportunityWriteGateRequired: false,
      publicLeadCreated: false,
      processedRealSystemJob: false,
      contactCreatedOrUpdated: false,
      opportunityCreatedOrUpdated: false,
      smsEmailSent: false,
      metaMutation: false,
...
export async function POST(request: Request) {
  try {
    ...
    assertWriteGates(body.proofRunId, body.mode);

    const admin = getAdminClient();
    if (body.mode === "workflowRetry") {
      return await runWorkflowRetryProof(admin);
    }
    ...
    const qaEmail = body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
      ? WORKFLOW_QA_EMAIL
      : body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
        ? qaLead.email
        : QA_EMAIL;
    const proofName = body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
      ? WORKFLOW_QA_NAME
      : body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
    ...
    const crmSyncResult = await safeSyncLeadToPartnerCrm({
      ...
      campaign_name: body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
        ...
        : body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
          ? "DealFlow controlled GHL workflow proof"
      ...
      source: qaLead.source || (body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
        ...
        : body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
          ? "ghl_workflow_v1_proof"
        : "live_ghl_contact_sync_proof"),
      lead_type: body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
        ? "qa_workflow_enrollment"
        : body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
          ? "qa_opportunity_sync"
          : "qa_contact_sync",
      ...
      metadata: {
        ...
        proof_type: body.proofRunId === OPPORTUNITY_V1_PROOF_RUN_ID
          ...
          : body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID
            ? "live_ghl_contact_opportunity_workflow_sync"
        ...
        stripe_billing_provider_action: false,
        provisioning: false,
        workflow_enrollment: body.proofRunId === WORKFLOW_V1_PROOF_RUN_ID,
      },
    });
    const afterCounts = await countRows(admin);
    ...
    return apiSuccess({
      ...
      safety: {
        ...
        provisioning: false,
        workflowEnrollment: Boolean((crmSyncResult as { workflowEnrollment?: unknown }).workflowEnrollment),
        tokensExposed: false,
        credentialRefsExposed: false,
      },
    }, {
