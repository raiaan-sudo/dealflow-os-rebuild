...
import { getSelfServeTrialPeriodDays, normalizeBillingPlanTier, type BillingPlanTier } from "@/lib/billing/plans";
...
async function loadPartnerBillingBundle(
  ...
  if (!normalizedSlug) {
    return { partnerId: null, partnerSlug: null, pricing: null, commissionRate: null };
  }
  ...
  if (brandingError) {
    throw new ApiError(500, brandingError.message, "partner_billing_pricing_fetch_failed");
  }

  const pricing = parsePartnerPricingConfig((branding as { pricing_json?: unknown } | null)?.pricing_json);
  return {
...
export async function createAccessKeyCheckoutSession(params: {
  ...
  const priceConfig =
    ...
    (planTier === "pro" ? getStripePlanPriceConfiguration(planTier, null) : null);

  if (!priceConfig) {
    throw new ApiError(
      503,
      partnerBilling.partnerId
        ? "The selected partner plan is not configured in Stripe."
        : "The selected plan is not configured in Stripe.",
      partnerBilling.partnerId ? "partner_stripe_price_missing" : "stripe_price_missing",
    );
  }
  ...
  const checkoutTrialPeriodDays = getSelfServeTrialPeriodDays(planTier);
  ...
  const metadata = {
    ...buildStripeCheckoutMetadata({
      ...
      partnerAttributionSource: "access_key_checkout",
...
export async function claimPendingAccessKeyForCurrentUser(context: AppContext) {
  ...
  const partnerId = row.partner_id ?? context.partner?.id ?? null;
  const partnerSlug = row.partner_slug ?? context.partner?.slug ?? null;
