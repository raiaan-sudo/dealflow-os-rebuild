...
function getLeadConsentSource(row: LeadRow | null) {
  const metadata = row?.consent_metadata && typeof row.consent_metadata === "object" && !Array.isArray(row.consent_metadata)
    ? row.consent_metadata
    : {};
...
function getNonBillableReason(params: {
  ...
  const consentSource = getLeadConsentSource(params.lead);
  if (consentSource && consentSource !== "public_lead_capture_form") {
    ...
    .upsert(
      {
        ...
        metadata: {
          ...
          consentSource: getLeadConsentSource(params.lead),
...
export async function runPerformanceLeadBillingJob(payload: PerformanceLeadBillingJobPayload) {
  ...
  const [{ data: leadData, error: leadError }, { data: billingData, error: billingError }] = await Promise.all([
    admin
      .from("leads")
      .select("id,organization_id,campaign_id,source,consent_metadata,created_at,metadata")
