...
export function validateWinningFunnel(value: unknown): WinningFunnelValidationResult {
  ...
  if (!Array.isArray(funnel.quizSteps) || funnel.quizSteps.length < 4) blockers.push("quiz_steps_missing");
  if (!Array.isArray(funnel.sections) || funnel.sections.length < 8) blockers.push("locked_sections_missing");
  ...
  if (!sectionVariants.has("locked-hero")) blockers.push("locked_hero_missing");
  if (!sectionVariants.has("native-multi-step-quiz")) blockers.push("native_quiz_missing");
  if (!sectionVariants.has("agent-authority")) blockers.push("agent_authority_missing");
  if (!sectionVariants.has("thank-you-state")) blockers.push("thank_you_state_missing");
