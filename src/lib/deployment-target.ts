...
// Synchronous and Edge-compatible. Vercel project IDs are restricted to
// printable ASCII, so this deliberately rejects any unexpected encoding.
...
export function isExactIsolatedStagingVercelHost(env: Record<string, string | undefined> = process.env) {
  return env.VERCEL_ENV?.trim().toLowerCase() === "production" &&
    ...
    sha256Ascii(env.VERCEL_PROJECT_ID?.trim() ?? "") ===
      DEALFLOW_CANONICAL_STAGING_VERCEL_PROJECT_ID_SHA256 &&
...
export function getDeploymentTarget(
  ...
  if (vercelEnvironment === "production") {
    ...
    return isExactProductionVercelHost(env) ? "production" : "unknown";
  }
