...
export function safeBrandAssetUrl(value: unknown) {
  const raw = optionalText(value);
  if (!raw) {
    return null;
  }
...
export function buildPartnerBranding(params: {
  ...
  const logoUrl = safeBrandAssetUrl(theme.logoUrl) ?? safeBrandAssetUrl(partner?.logo_url);
  const faviconUrl = safeBrandAssetUrl(theme.faviconUrl) ?? safeBrandAssetUrl(partner?.favicon_url);
  ...
  const isPartnerBrand = Boolean(partner?.id);
  ...
  return {
    ...
    supportFooterCopy:
      ...
      (isPartnerBrand ? `Support is handled by ${brandName}.` : DEFAULT_PARTNER_BRANDING.supportFooterCopy),
    poweredByDealFlow: partner?.powered_by_dealflow ?? true,
    loginEyebrow:
      optionalText(copy.loginEyebrow) ??
      (isPartnerBrand ? `${brandName} platform` : DEFAULT_PARTNER_BRANDING.loginEyebrow),
    loginHeadline:
      optionalText(copy.loginHeadline) ??
      (isPartnerBrand ? `Welcome to ${brandName}` : DEFAULT_PARTNER_BRANDING.loginHeadline),
    loginSubheadline:
      optionalText(copy.loginSubheadline) ??
      (isPartnerBrand
        ? `Sign in to build your campaign, creative, launch path, and follow-up workflow in the ${brandName} workspace.`
        : DEFAULT_PARTNER_BRANDING.loginSubheadline),
    pricing,
    featureFlags,
  };
}
